...
serve(async (req: Request) => {
  ...
  try {
    ...
    reversedHistory.forEach((log: any) => {
    ...
    const hudInstruction = showHud 
      ? `AFFICHE CE HUD AU DÉBUT DE TA RÉPONSE :\n| 👤 Élève | 📊 Niveau | 🎯 Objectif | 🎭 Mode |\n| :--- | :--- | :--- | :--- |\n| ${userName} | ${currentLevel} | "${rawObjective}" | ${currentStyle} |\n\n` 
      : `N'AFFICHE PAS LE TABLEAU/HUD DANS CE MESSAGE.`

    // 5. LE PROMPT COMPLET
    const SYSTEM_PROMPT = `
    Tu es "NorvegeIA", le coach de ${userName} (Niveau ${currentLevel}).
    ...
    **FORMAT TECHNIQUE OBLIGATOIRE (JSON à la fin) :**
    ...
    if (saveToLog) {
      await supabaseClient.from('learning_logs').insert({
        ...
        content: { user_msg: userMessage, ai_response: fullText, style_used: currentStyle }
      })
    }

    // 7. RETOUR (Brut, Flutter nettoie)
    return new Response(
      JSON.stringify({ reply: fullText, metadata: {} }),
